...
namespace Control_IMU_JACO
{
	...
	{
		///////////////      T     R     I      S      T       A      N       ///////////////////////////////////////
		if(y0Cal-HEAD_IMU.rollraw >= 5){
		Tristan_CmdY_new = CTRL_IMU.y0Min_LinCoef*(y0Cal - HEAD_IMU.rollraw); //Map angle to CmdY
		...
		Tristan_CmdY_new = CTRL_IMU.y0Max_LinCoef*(HEAD_IMU.rollraw - y0Cal); //Map angle to CmdY
			if (Tristan_CmdY_new > MAX_SPEED){
				Tristan_CmdY_new = MAX_SPEED; //Limits CmdY to 2.5
			}
		} else {
		...
		if(x0Cal-HEAD_IMU.yawraw >= 2.25){
		Tristan_CmdX_new = CTRL_IMU.x0Min_LinCoef*(x0Cal-HEAD_IMU.yawraw); //Map angle to CmdX
		...
		if(HEAD_IMU.yawraw - x0Cal >= 2.25){
		Tristan_CmdX_new = CTRL_IMU.x0Max_LinCoef*(HEAD_IMU.yawraw - x0Cal); //Map angle to CmdX
			if(Tristan_CmdX_new > MAX_SPEED){
				Tristan_CmdX_new = MAX_SPEED //Limits CmdX to 2.5
			}
		} else {
