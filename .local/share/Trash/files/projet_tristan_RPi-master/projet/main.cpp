...
int main()
{
  ...
  while (1)
  {
	...
	if (data[Packet_Size-2] == 0x34)
	{
      ...
      CTRL_IMU.x0Max = CTRL_IMU.HEAD_IMU.yawraw;
		...
		if (CTRL_IMU.calDone > 100 && CTRL_IMU.calDone <= 500){ //Then we run this second if() --> Min./Max. calibration
			...
			CTRL_IMU.y0Min_LinCoef = (CTRL_IMU.y0Cal - CTRL_IMU.y0Min)/-MAX_SPEED; //Compute linear coefficient to map Y angle to speed
			}
			...
			CTRL_IMU.x0Min_LinCoef = (CTRL_IMU.x0Cal - CTRL_IMU.x0Min)/-MAX_SPEED; //Compute linear coefficient to map X angle to speed
			}
	...
	}while(CTRL_IMU.Jaco_Init == 0); */

	if (CTRL_IMU.calDone > 499){ //Then we go on the regular while(1) loop
