...
namespace Control_IMU_JACO
{
	...
	{
		ZActive = 1;
		ReferenceActive = 0;						//indique l'acitivation du noeud de r�f�rence
		Valid = 1;									//indique la validit� des marges et zones d�finies
		DiagoActive = 1;							//0 = diagonale d�sactiv�e, 1 = diagonale activ�e
		Theta_offset = 0;							//0 .. 360 degr�s, mapping
	...
	void Control_JACO::finger_ctrl(void)
	{
		///////////////      T     R     I      S      T       A      N       ///////////////////////////////////////
		if(HEAD_IMU.rollraw <= (y0Cal-5)){
		Tristan_CmdY_new = (y0Cal - HEAD_IMU.rollraw)/y0Min_LinCoef; //Map angle to CmdY
			if (Tristan_CmdY_new < (-1*MAX_SPEED)){
				Tristan_CmdY_new = (-1*MAX_SPEED); //Limits CmdY to -2.5
			}
		}else if (HEAD_IMU.rollraw - y0Cal >= 5){
		Tristan_CmdY_new = (HEAD_IMU.rollraw - y0Cal)/y0Max_LinCoef; //Map angle to CmdY
		if (Tristan_CmdY_new > MAX_SPEED){
			Tristan_CmdY_new = MAX_SPEED; //Limits CmdY to 2.5
			}
		}
		else 
		{
		...
		if(HEAD_IMU.yawraw <= (y0Cal-2.25)){
		Tristan_CmdX_new = (x0Cal - HEAD_IMU.yawraw)/x0Min_LinCoef; //Map angle to CmdX
			if (Tristan_CmdX_new < (-1*MAX_SPEED)){
				Tristan_CmdX_new = (-1*MAX_SPEED); //Limits CmdX to -2.5
			}
		}else if (HEAD_IMU.yawraw - x0Cal >= 2.25){
			Tristan_CmdX_new = (HEAD_IMU.yawraw - x0Cal)/x0Max_LinCoef; //Map angle to CmdX
				if(Tristan_CmdX_new > MAX_SPEED){
				Tristan_CmdX_new = MAX_SPEED; //Limits CmdX to 2.5 
			}
		} else {
		Tristan_CmdX_new = 0;
		}
